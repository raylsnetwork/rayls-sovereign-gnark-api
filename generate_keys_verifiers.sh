        ...
        if [[ -n "$target_name" ]]; then
            ...
            if [[ "$final_basename" == "EnygmaJoinSplitVerifier" ]] || \
               ...
               [[ "$final_basename" == "Erc1155JoinSplitVerifier" ]]; then
                target_dir="$HOME/$BASE_DIR/rayls-privacy-contracts/src/rayls-protocol/Enygma/Enygma-DVP"
                echo "File matches DVP pattern, routing to Enygma-DVP directory"
            else
                target_dir="$HOME/$BASE_DIR/rayls-privacy-contracts/src/rayls-protocol/Enygma/Enygma-Payments"
