...
var (
	erc20CircuitCache     *CompiledCircuit
	erc20CircuitCacheOnce sync.Once
...
// Circuit loading function for Enygma join-split
func getOrLoadEnygmaCircuit(pkPath, r1csPath string) *CompiledCircuit {
	erc20CircuitCacheOnce.Do(func() {
		fmt.Println("Loading pre-compiled Enygma join-split circuit...")
		start := time.Now()

		r1cs, err := loadConstraintSystem(r1csPath)
		if err != nil {
			panic(fmt.Sprintf("Failed to load Enygma constraint system: %v", err))
		}

		pk, err := primitives.LoadProvingKey(ecc.BN254, pkPath)
		if err != nil {
			panic(fmt.Sprintf("Failed to load Enygma proving key: %v", err))
		}
		...
		fmt.Printf("✓ Enygma Circuit loaded and cached in %v\n", time.Since(start))
	})
	return erc20CircuitCache
}

// Handler function for Enygma join-split
func NewHandler(pkPath, vkPath, r1csPath string) gin.HandlerFunc {
	// Pre-load the circuit
	compiled := getOrLoadEnygmaCircuit(pkPath, r1csPath)

	return func(c *gin.Context) {
		totalStart := time.Now()

		var request EnygmaJoinSplitRequest
		...
		var witness EnygmaJoinSplitCircuit
		setEnygmaWitnessOptimized(&witness, &request)
		parseTime := time.Since(totalStart)

		response, err := generateEnygmaProof(&witness, compiled, &request)
		if err != nil {
		...
		fmt.Printf("Enygma Join-Split Timing: Bind=%v, Parse=%v, Prove=%v, Total=%v\n",
...
// Input validation for Enygma join-split
func validateEnygmaInputs(request *EnygmaJoinSplitRequest) error {
	...
	if len(request.KeyPairsIn) != len(request.ValuesIn) {
		return fmt.Errorf("number of key pairs (%d) must match number of input values (%d)", len(request.KeyPairsIn), len(request.ValuesIn))
	}

	if len(request.MerkleProofs) != len(request.ValuesIn) {
		return fmt.Errorf("number of merkle proofs (%d) must match number of input values (%d)", len(request.MerkleProofs), len(request.ValuesIn))
	}

	if len(request.MerkleRoots) != len(request.ValuesIn) {
		return fmt.Errorf("number of merkle roots (%d) must match number of input values (%d)", len(request.MerkleRoots), len(request.ValuesIn))
	}
...
// Set witness for Enygma circuit
func setEnygmaWitnessOptimized(witness *EnygmaJoinSplitCircuit, request *EnygmaJoinSplitRequest) {
	//fmt.Println("=== WITNESS PREPARATION START ===")
	//fmt.Printf("NFT Commitment: %s\n", request.NftCommitment)
	//fmt.Printf("Contract Address (hex): %s\n", request.EnygmaContractAddress)

	witness.NftCommitment = parseBigIntOptimized(request.NftCommitment)
	witness.EnygmaContractAddress = parseBigIntOptimized(convertAddressToBigInt(request.EnygmaContractAddress))

	// Fill actual inputs
	actualInputs := len(request.ValuesIn)
	//fmt.Printf("\n=== Processing %d actual inputs ===\n", actualInputs)
	...
	// Fill remaining inputs with dummy values (zeros)
	...
	for i := 0; i < mOutputs; i++ {
		...
		// Generate V2 output commitment: H(H(H(recipientPK, salt), amount), tokenAddress)
		contractAddrStr := convertAddressToBigInt(request.EnygmaContractAddress)
		outputCommitment := primitives.ComputeCommitmentV2ERC20BN254(
			request.PubKeysOut[i].PublicKey, request.SaltsOut[i],
			request.ValuesOut[i], contractAddrStr,
		)
	...
	// Compute revert commitment using the total input amount and same contract address
	contractAddrStrRevert := convertAddressToBigInt(request.EnygmaContractAddress)
	inputTotal := new(big.Int)
	for _, v := range request.ValuesIn {
		val, _ := new(big.Int).SetString(v, 10)
		inputTotal.Add(inputTotal, val)
	}
	revertCommitment := primitives.ComputeCommitmentV2ERC20BN254(
		senderPK, request.RevertSalt,
		inputTotal.String(), contractAddrStrRevert,
	)
...
func computeUniqueId(contractAddress, value string) string {
	return primitives.ComputeUniqueIdBN254(contractAddress, value)
}

// Generate proof for Enygma circuit
func generateEnygmaProof(witness frontend.Circuit, compiled *CompiledCircuit, request *EnygmaJoinSplitRequest) (*EnygmaJoinSplitResponseAPI, error) {
	response, _, err := generateProofGeneric(witness, compiled)
	if err != nil {
		return nil, err
	}

	enygmaResponse := &EnygmaJoinSplitResponseAPI{
		Pi_A:          response.Pi_A,
		Pi_B:          response.Pi_B,
		Pi_C:          response.Pi_C,
		Public_Signal: generateEnygmaPublicSignal(request),
	}

	return enygmaResponse, nil
}

// Generate public signals for Enygma circuit
func generateEnygmaPublicSignal(request *EnygmaJoinSplitRequest) []string {
	publicSignal := make([]string, 0, 1+nInputs+nInputs+nInputs+mOutputs) // nftCommitment + merkleRoots + nullifiers + treeNumbers + commitmentsOut
...
func generateProofGeneric(witness frontend.Circuit, compiled *CompiledCircuit) (*EnygmaJoinSplitResponseAPI, []string, error) {
	...
	response := &EnygmaJoinSplitResponseAPI{
		Pi_A: []string{
			A_x1.String(),
			A_y1.String(),
		},
